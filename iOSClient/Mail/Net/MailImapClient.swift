...
/// Authentication failure marker: the message starts with "[AUTH]" so the
/// view model can trigger the credential self-heal.
struct MailAuthError: LocalizedError {
    let underlying: Error

    var errorDescription: String? {
        "[AUTH] Authentication failed: \(underlying.localizedDescription)"
    }
}
...
actor MailImapClient {
    ...
    private func withConnection<T>(_ body: (MailNIOConnection) async throws -> T) async throws -> T {
        ...
        do {
            _ = try await connection.send(.login(username: account.saslUser, password: account.mailPassword))
        } catch {
            // Mark authentication failures so the view model can re-mint the
            // combined app password and retry once.
            throw MailAuthError(underlying: error)
        }
    ...
    func send(fromAddress: String, fromName: String, outgoing: OutgoingMessage, sentPath: String?) async -> MailResult<Void> {
        ...
        let smtp = MailSmtpClient(host: account.host, port: smtpPort, username: account.saslUser, password: account.mailPassword, group: group)
        do {
